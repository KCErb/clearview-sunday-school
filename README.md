...
- Every lesson gets the standard **Join the class** slide after its title slide (`--no-join` to
  skip): a QR code for the site plus how to answer and follow along. Hub links are renumbered so
  they still land on the same slides. The presenter view has a one-tap **Join slide** button for
  latecomers, and on the TV any slide with an attached question carries a small QR badge.
  `public/join-qr.svg` is generated by `node scripts/join-qr.mjs`.
